...
type Talep = {
  ...
  ise_baslama_zamani?: string; ise_bitis_zamani?: string;
  toplam_km?: number;
  foto_teslim_alma?: string[]; foto_yukleme?: string[]; foto_teslim?: string[];
};
type Adim = "liste" | "detay" | "yolda" | "teslim_alma" | "yukleme" | "teslimat" | "teslim_foto" | "ozet";

const DB_TO_ADIM: Record<string, Adim> = {
  yolda: "yolda", arac_yaninda: "teslim_alma",
  yukleniyor: "yukleme", teslimat_yolunda: "teslimat",
  teslimatta: "teslim_foto", tamamlandi: "ozet",
};

const ADIM_SIRALAMA: Adim[] = ["yolda", "teslim_alma", "yukleme", "teslimat", "teslim_foto"];
const ADIM_ETIKET: Record<string, string> = {
  yolda: "Yolda", teslim_alma: "Teslim Alma", yukleme: "Yükleme",
  teslimat: "Teslimat", teslim_foto: "Teslim Foto",
};
...
export default function SoforPanel() {
  ...
  const [fotolar, setFotolar] = useState({
    ...
    yukleme: [null, null] as (string | null)[],
  ...
  const [aktifSlot, setAktifSlot] = useState<{ step: "teslim_alma" | "yukleme" | "teslim"; idx: number } | null>(null);
  ...
  const taleplerYukle = useCallback(async (soforId: string) => {
    const { data } = await supabase
      .from("talepler")
      .select("id, tip, durum, is_adim, created_at, musteri_id, musteri_ad, musteri_tel, arac_plaka, konum_lat, konum_lng, konum_adres, hedef_adres, aciklama, ise_baslama_zamani, ise_bitis_zamani, toplam_km, foto_teslim_alma, foto_yukleme, foto_teslim")
  ...
  function talepSec(t: Talep) {
    ...
    setToplamKm(km);
    toplamKmRef.current = km;
    setFotolar({
      teslim_alma: pad(t.foto_teslim_alma as (string|null)[] || [], 4),
      yukleme: pad(t.foto_yukleme as (string|null)[] || [], 2),
      teslim: pad(t.foto_teslim as (string|null)[] || [], 4),
    });
    if (t.durum === "tamamlandi" || t.is_adim === "tamamlandi") {
      setAdim("ozet");
    } else {
      const yeniAdim = t.is_adim ? (DB_TO_ADIM[t.is_adim] || "detay") : "detay";
      setAdim(yeniAdim);
      if (yeniAdim === "yolda" || yeniAdim === "teslimat") {
        kmAktifRef.current = true;
        startGPS(t.id);
      }
    }
  }
  ...
  useEffect(() => {
    if (!seciliTalep?.ise_baslama_zamani || adim === "ozet" || adim === "liste" || adim === "detay") return;
  ...
  async function iseBasla() {
    ...
    const { error } = await supabase.from("talepler").update({
      durum: "yolda", is_adim: "yolda",
      ise_baslama_zamani: new Date().toISOString(),
      baslangic_lat: lat || null, baslangic_lng: lng || null,
      sofor_konum_lat: lat || null, sofor_konum_lng: lng || null,
      sofor_konum_updated_at: new Date().toISOString(),
    }).eq("id", seciliTalep.id);
    setIslemYapiliyor(false);
    if (error) { setHata(error.message); return; }
    const updated = { ...seciliTalep, durum: "yolda", is_adim: "yolda", ise_baslama_zamani: new Date().toISOString() };
    setSeciliTalep(updated);
    setAdim("yolda");
    kmAktifRef.current = true;
    startGPS(seciliTalep.id);
  }

  async function sonrakiAdim(dbAdim: string, yerelAdim: Adim, extraData?: Record<string, unknown>) {
    if (!seciliTalep) return;
    setIslemYapiliyor(true);
    await supabase.from("talepler").update({ is_adim: dbAdim, ...extraData }).eq("id", seciliTalep.id);
    setIslemYapiliyor(false);
    setAdim(yerelAdim);
  }
  ...
  async function araciYukle() {
    stopGPS();
    const urls = fotolar.teslim_alma.filter(Boolean) as string[];
    await sonrakiAdim("yukleniyor", "yukleme", { foto_teslim_alma: urls });
  }

  async function teslimatYoluna() {
    const urls = fotolar.yukleme.filter(Boolean) as string[];
    await sonrakiAdim("teslimat_yolunda", "teslimat", { foto_yukleme: urls });
    kmAktifRef.current = true;
    startGPS(seciliTalep!.id);
  }
  ...
  async function teslimEttim() {
    stopGPS();
    await sonrakiAdim("teslimatta", "teslim_foto");
  }

  async function isiTamamla() {
    if (!seciliTalep) return;
    setIslemYapiliyor(true);
    const urls = fotolar.teslim.filter(Boolean) as string[];
    const bitisTarih = new Date().toISOString();
    await supabase.from("talepler").update({
      durum: "tamamlandi", is_adim: "tamamlandi",
      ise_bitis_zamani: bitisTarih,
      foto_teslim: urls, toplam_km: toplamKmRef.current,
    }).eq("id", seciliTalep.id);
    setIslemYapiliyor(false);
    setSeciliTalep(prev => prev ? { ...prev, ise_bitis_zamani: bitisTarih } : prev);
    setAdim("ozet");
  ...
  async function handleFotoSecme(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const path = `talepler/${seciliTalep.id}/${aktifSlot.step}/${Date.now()}_${aktifSlot.idx}.jpg`;
    const { data, error } = await supabase.storage.from("belgeler").upload(path, file, { contentType: file.type || "image/jpeg", upsert: true });
    setFotoYukleniyor(false);
    if (error) { setHata("Fotoğraf yüklenemedi: " + error.message); }
    else {
      const { data: { publicUrl } } = supabase.storage.from("belgeler").getPublicUrl(data.path);
      setFotolar(prev => {
        const arr = [...prev[aktifSlot.step]];
        arr[aktifSlot.idx] = publicUrl;
        return { ...prev, [aktifSlot.step]: arr };
      });
      setHata("");
    }
  ...
  function fotoSec(step: "teslim_alma" | "yukleme" | "teslim", idx: number) {
  ...
  function FotoGrid({ step, labels, count }: { step: "teslim_alma" | "yukleme" | "teslim"; labels: string[]; count: number }) {
      ...
      <>
        {fotoYukleniyor && (
          <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-3 mb-4 text-xs text-blue-300 flex items-center gap-2">
            <span className="animate-spin inline-block">⏳</span> Fotoğraf yükleniyor...
          </div>
        )}
        <div className={`grid gap-3 mb-4 ${labels.length === 2 ? "grid-cols-2" : "grid-cols-2"}`}>
          {labels.map((label, i) => (
            <div key={label} onClick={() => fotoSec(step, i)}
              className={`aspect-square rounded-2xl border-2 flex flex-col items-center justify-center cursor-pointer transition overflow-hidden ${arr[i] ? "border-[#00C853]" : "border-dashed border-white/20 hover:border-[#FF4D00] bg-[#1A1A1A]"}`}>
              {arr[i] ? (
                ...
                <><span className="text-3xl mb-1">📷</span><span className="text-[11px] font-bold text-gray-500 text-center px-2">{label}</span></>
              )}
            </div>
          ))}
        </div>
        <div className="text-center text-xs text-gray-500 mb-5">
          <span className={`font-black text-base ${arr.filter(Boolean).length === count ? "text-[#00C853]" : "text-[#FF4D00]"}`}>
            {arr.filter(Boolean).length}
          </span>/{count} fotoğraf çekildi
        </div>
      </>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-[#0D0D0D] text-white flex flex-col">
      ...
      {/* Adım göstergesi */}
      {adim !== "liste" && adim !== "detay" && adim !== "ozet" && (
              ...
              return (
                <div key={s} className="flex items-center flex-1">
                  <div className="flex flex-col items-center flex-shrink-0">
                    ...
                    <div className={`text-[8px] font-bold mt-0.5 hidden sm:block ${aktif ? "text-[#FF4D00]" : gecildi ? "text-[#00C853]" : "text-gray-600"}`}>{ADIM_ETIKET[s]}</div>
                  </div>
                  {i < ADIM_SIRALAMA.length - 1 && (
                    <div className={`flex-1 h-0.5 mx-0.5 mb-3 ${gecildi ? "bg-[#00C853]" : "bg-white/10"}`}></div>
                  )}
                </div>
              );
            })}
      ...
      <input ref={fotoInputRef} type="file" accept="image/*" capture="environment" onChange={handleFotoSecme} className="hidden" />
      ...
      <div className="flex-1 overflow-y-auto p-4 pb-10">
        ...
        {adim === "detay" && seciliTalep && (
          <div>
            ...
            <div className="bg-[#1A1A1A] border border-white/8 rounded-2xl p-4 mb-3">
              ...
              {seciliTalep.musteri_ad && (
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-10 h-10 rounded-full bg-[#2A2A2A] flex items-center justify-center font-bold">{seciliTalep.musteri_ad[0]}</div>
            ...
            {seciliTalep.konum_lat && (
              <a href={`https://maps.google.com/?q=${seciliTalep.konum_lat},${seciliTalep.konum_lng}`} target="_blank" rel="noopener noreferrer"
                className="bg-[#1A1A1A] border border-white/8 rounded-2xl p-4 mb-3 flex items-center gap-3">
                <span className="text-2xl">📍</span>
                <div className="flex-1">
                  <div className="text-[10px] text-gray-500 uppercase font-bold">Aracın Yeri</div>
                  <div className="text-sm font-semibold text-[#FF4D00]">{seciliTalep.konum_adres || `${seciliTalep.konum_lat?.toFixed(5)}, ${seciliTalep.konum_lng?.toFixed(5)}`}</div>
                </div>
                <span className="text-gray-400">→</span>
              </a>
            )}
            {seciliTalep.hedef_adres && (
              <div className="bg-[#1A1A1A] border border-white/8 rounded-xl p-3 mb-3 flex items-center gap-2">
                <span>🎯</span><div><div className="text-[10px] text-gray-500 uppercase font-bold">Hedef</div><div className="text-sm font-semibold">{seciliTalep.hedef_adres}</div></div>
              </div>
            )}
            ...
            <button onClick={iseBasla} disabled={islemYapiliyor}
        ...
        {adim === "yolda" && seciliTalep && (
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-5">
              <div className="bg-[#1A1A1A] border border-white/8 rounded-xl p-4 text-center">
                <div className="text-2xl font-black text-[#FF4D00]">{gecenSure}</div>
                <div className="text-[10px] text-gray-500 mt-1">Geçen Süre</div>
              </div>
              <div className="bg-[#1A1A1A] border border-white/8 rounded-xl p-4 text-center">
                <div className="text-2xl font-black">{toplamKm.toFixed(1)}<span className="text-sm font-normal text-gray-500"> km</span></div>
                <div className="text-[10px] text-gray-500 mt-1">Kilometre</div>
              </div>
            </div>
            {sonKonum && (
              <div className="bg-blue-500/8 border border-blue-500/20 rounded-xl p-3 mb-4 text-xs text-blue-300 flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-blue-400 animate-pulse flex-shrink-0"></span>
                Konumunuz müşteriye iletiliyor · {sonKonum.lat.toFixed(4)}, {sonKonum.lng.toFixed(4)}
              </div>
            )}
            {seciliTalep.musteri_tel && (
              <a href={`tel:${tel(seciliTalep.musteri_tel)}`} className="flex items-center gap-3 bg-[#1A1A1A] border border-white/8 rounded-xl p-3 mb-3">
                <span className="text-xl">📞</span>
                <div><div className="text-[10px] text-gray-500">Müşteri</div><div className="font-bold text-blue-400">{seciliTalep.musteri_ad} · {tel(seciliTalep.musteri_tel)}</div></div>
              </a>
            )}
            {seciliTalep.konum_lat && (
              <a href={`https://maps.google.com/?q=${seciliTalep.konum_lat},${seciliTalep.konum_lng}`} target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-3 bg-[#1A1A1A] border border-white/8 rounded-xl p-3 mb-6">
                <span className="text-xl">📍</span>
                <div className="flex-1"><div className="text-[10px] text-gray-500">Müşterinin Konumu</div><div className="text-sm font-semibold text-[#FF4D00]">{seciliTalep.konum_adres || "Haritada Aç"}</div></div>
                <span className="text-gray-400">→</span>
              </a>
            )}
            <button onClick={() => sonrakiAdim("arac_yaninda", "teslim_alma")}
              className="w-full bg-[#FF4D00] hover:bg-[#CC3D00] text-white font-bold py-4 rounded-xl transition text-sm">
              📍 Araç Yanına Vardım
            </button>
          </div>
        )}

        {/* TESLİM ALMA FOTOĞRAFLARI */}
        {adim === "teslim_alma" && seciliTalep && (
          <div>
            <div className="font-black text-lg mb-1">📸 Teslim Alma</div>
        ...
        {adim === "yukleme" && seciliTalep && (
          <div>
            <div className="font-black text-lg mb-1">🔗 Yükleme</div>
            <div className="text-gray-500 text-xs mb-4">Araç bağlandıktan sonra fotoğraf çekin.</div>
            <FotoGrid step="yukleme" labels={["Genel Görünüm", "Bağlantı Noktası"]} count={2} />
            <div className="bg-[#00C853]/6 border border-[#00C853]/15 rounded-xl p-3 text-xs text-green-300 mb-4">
              📱 Müşteriye &ldquo;Araç yüklendi, teslimata gidiyorum&rdquo; bildirimi gönderilecek.
            </div>
            <button onClick={teslimatYoluna} disabled={fotolar.yukleme.filter(Boolean).length < 2 || islemYapiliyor}
              className="w-full bg-[#FF4D00] hover:bg-[#CC3D00] disabled:opacity-40 text-white font-bold py-4 rounded-xl transition text-sm">
              🚛 Teslimata Git →
            </button>
          </div>
        ...
        {adim === "teslimat" && seciliTalep && (
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-5">
              <div className="bg-[#1A1A1A] border border-white/8 rounded-xl p-4 text-center">
                <div className="text-2xl font-black text-[#FF4D00]">{gecenSure}</div>
                <div className="text-[10px] text-gray-500 mt-1">Toplam Süre</div>
              </div>
              <div className="bg-[#1A1A1A] border border-white/8 rounded-xl p-4 text-center">
                <div className="text-2xl font-black">{toplamKm.toFixed(1)}<span className="text-sm font-normal text-gray-500"> km</span></div>
                <div className="text-[10px] text-gray-500 mt-1">Kilometre</div>
            ...
            {seciliTalep.hedef_adres && (
              <div className="bg-[#1A1A1A] border border-white/8 rounded-xl p-3 mb-4 flex items-center gap-2">
                <span>🎯</span><div><div className="text-[10px] text-gray-500 uppercase font-bold">Hedef</div><div className="text-sm font-semibold">{seciliTalep.hedef_adres}</div></div>
              </div>
            )}
            {sonKonum && (
              <div className="bg-blue-500/8 border border-blue-500/20 rounded-xl p-3 mb-6 text-xs text-blue-300 flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-blue-400 animate-pulse flex-shrink-0"></span>
                GPS aktif · {sonKonum.lat.toFixed(4)}, {sonKonum.lng.toFixed(4)}
              </div>
            )}
            <button onClick={teslimEttim}
              className="w-full bg-[#00C853] hover:bg-[#00a844] text-black font-bold py-4 rounded-xl transition text-sm">
              ✅ Teslim Ettim
            </button>
          </div>
        )}
        ...
        {adim === "teslim_foto" && seciliTalep && (
          <div>
            ...
            <button onClick={isiTamamla} disabled={fotolar.teslim.filter(Boolean).length < 4 || islemYapiliyor}
          ...
          <div className="text-center">
            ...
            <div className="bg-[#1A1A1A] border border-white/8 rounded-2xl p-5 text-left space-y-3 mb-6">
              <div className="flex items-center gap-3 pb-3 border-b border-white/5">
                <span className="text-xl">👤</span>
                <div className="flex-1"><div className="text-xs text-gray-500">Müşteri</div><div className="font-bold">{seciliTalep.musteri_ad || "—"}</div></div>
              </div>
              <div className="flex items-center gap-3 pb-3 border-b border-white/5">
                <span className="text-xl">🚗</span>
                <div className="flex-1"><div className="text-xs text-gray-500">Araç</div><div className="font-bold">{seciliTalep.arac_plaka || "—"}</div></div>
              </div>
              <div className="grid grid-cols-2 gap-4 pb-3 border-b border-white/5">
                <div>
                  <div className="text-xs text-gray-500">Toplam KM</div>
                  <div className="font-black text-xl text-[#FF4D00]">{toplamKm.toFixed(1)} km</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500">Süre</div>
                  <div className="font-black text-xl">
                    {seciliTalep.ise_baslama_zamani && seciliTalep.ise_bitis_zamani
                      ? formatSure(new Date(seciliTalep.ise_bitis_zamani).getTime() - new Date(seciliTalep.ise_baslama_zamani).getTime())
                      : gecenSure}
                  </div>
                </div>
              </div>
              <div className="grid grid-cols-3 gap-2 text-center">
                {[
                  ...
                  <div key={f.l} className="bg-[#2A2A2A] rounded-xl p-3">
                    <div className="font-black text-xl text-[#00C853]">{f.c}</div>
