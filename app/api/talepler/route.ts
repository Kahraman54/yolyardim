...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await supabaseAdmin.from("talepler").insert(body).select().single();
...
export async function PATCH(req: NextRequest) {
  ...
  const { data, error } = await supabaseAdmin.from("talepler").update(updates).eq("id", id).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json(data);
}
...
export async function GET(req: NextRequest) {
  ...
  let query = supabaseAdmin.from("talepler").select("*").order("created_at", { ascending: false });
